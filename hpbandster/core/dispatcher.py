...
class Dispatcher(object):
    ...
    def __init__(self, new_result_callback, run_id='0',
                 ...
                 host=None, logger=None, queue_callback=None, monitor='0.0.0.0',
                 monitor_port=0, ):
        """
        Parameters
        ----------
        ...
        while True:
            ...
            for wn in all_workers:
                ...
                if not self.worker_pool[wn].is_alive():
                    ...
                    # new thing. Notify monitor to delete worker
                    client_socket = socket.socket()  # instantiate
                    client_socket.connect((self.monitor, self.monitor_port))  # connect to the monitor
                    message = 'addworker!' + self.run_id + '!0!0!-1'
                    client_socket.send(message.encode())
